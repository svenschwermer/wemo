...
int mtd_write_from_file (const char *mtd_device, const char *file)
{
    ...
    memcpy(&buf[0], &mtd_hdr, sizeof(mtd_hdr));
    if (file) {
        int n, ct;
    
        n = read(in_fd, &buf[sizeof(mtd_hdr)], content_size);
        ct = write(mtd_fd, &buf[0], n+sizeof(mtd_hdr));
        if (-1 == ct) {
            // perror("mtd_write: mtd buffer write()");
            rc = ERR_IO_MTD_WRITE;
            goto err_mtd_write;
        }
        if (ct != n+sizeof(mtd_hdr)) {
...
int mtd_read_to_file (const char *mtd_device, const char *file)
{
    ...
    if (-1 == (out_fd = open(file, O_CREAT | O_WRONLY | O_TRUNC, 0644))) {
