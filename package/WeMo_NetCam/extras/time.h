...
/* Defined in localtime.c(this localtime.c is in tzcode,is not uclibc code)(add by ka 2013.6.9)  */
__BEGIN_NAMESPACE_STD
struct dumptz
{
	int  ishaddst;	//if this is 0,beginSec=endSec=0
	long beginSec;	//the time since the Epoch(00:00:00 UTC, January 1, 1970) start daylight saving time 
	long endSec;  	//the time since the Epoch(00:00:00 UTC, January 1, 1970) stop daylight saving time
	long gmtoff;  	//Seconds east of UTC 
	long dstgmtoff;	//Seconds east of UTC while start daylight saving time 
};
extern struct dumptz *dumptimezone(int year,char *dumptimezone);
__END_NAMESPACE_STD
#if defined __USE_XOPEN || defined __USE_POSIX || defined __USE_MISC
__USING_NAMESPACE_STD(dumptz)
#endif
