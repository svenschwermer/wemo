...
{
  //The counter thresholds took too much space on RAM. Until
  //it is changed to a plugin, this code has been commented out
  //with #define EMBER_APPLICATION_HAS_ROLLOVER_HANDLER
...
#if defined(EMBER_APPLICATION_HAS_COUNTER_ROLLOVER_HANDLER) && !defined(EZSP_HOST)
    if(emberCounters[type]>=emberCountersThresholds[type])
    {
      emberCounterRolloverHandler(type);
      emberCounters[type] = 0;
    }
#endif
