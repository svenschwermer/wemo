...
extern void zbdSysEventOtaNoNextImage();
                                          ...
                                          int16u* hardwareVersion)
{
  ...
  prepareClusterResponse(ZCL_QUERY_NEXT_IMAGE_RESPONSE_COMMAND_ID,
                         status);
  fprintf(stderr, "ZCL_QUERY_NEXT_IMAGE_RESPONSE_COMMAND_ID:status:0x%02X\n",status);
  if (status != EMBER_ZCL_STATUS_SUCCESS) {
	  if (status == EMBER_ZCL_STATUS_NO_IMAGE_AVAILABLE)
	  {
		  zbdSysEventOtaNoNextImage();
	  }
    return status;
  }
