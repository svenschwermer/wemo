...
#include "tou-calendar-common-callback.h"

//-----------------------------------------------------------------------------
// Globals

const char calendarName0[] = "Grumpy Cat Calendar (0)";
const char calendarName1[] = "Jumpy Dog Calendar (1)";
...
#define MAX_CALENDAR_TYPE EMBER_ZCL_SSWG_CALENDAR_TYPE_AUXILLIARY_LOAD_SWITCH_CALENDAR
...
{
  EmberAfTouCalendarStruct* calendar = emberAfPluginTouCalendarCommonGetLocalCalendar(calendarIndexForPrinting);
...
{
  switch (calendarType) {
    case EMBER_ZCL_SSWG_CALENDAR_TYPE_DELIVERED_CALENDAR:
    case EMBER_ZCL_SSWG_CALENDAR_TYPE_RECEIVED_CALENDAR:
    case EMBER_ZCL_SSWG_CALENDAR_TYPE_DELIVERED_AND_RECEIVED_CALENDAR:
      emberAfTouCalendarClusterPrint("Price Tier: 0x%X", data);
      break;
    case EMBER_ZCL_SSWG_CALENDAR_TYPE_FRIENDLY_CREDIT_CALENDAR:
      emberAfTouCalendarClusterPrint("Friendly credit: %p", 
                                     data == 0
                                     ? "not available"
                                     : "enabled");
      break;
    case EMBER_ZCL_SSWG_CALENDAR_TYPE_AUXILLIARY_LOAD_SWITCH_CALENDAR:
...
{
  emberAfTouCalendarClusterPrintln("Day ID: %d", specialDay->day->id);
...
static void printCalendar(const EmberAfTouCalendarStruct* calendar)
{
  ...
  emberAfTouCalendarClusterPrintln("Name: %p", calendar->name);
...
static void setupTestDays(EmberAfTouCalendarStruct* calendar)
{
  ...
  for (i = 0; i < calendar->numberOfSpecialDayProfiles; i++) {
    ...
    calendar->specialDays[i].day = &(calendar->normalDays[6]);
  }
}
...
static void setupTestWeeks(EmberAfTouCalendarStruct* calendar)
{
  ...
  for (i = 0; i < calendar->numberOfWeekProfiles; i++) {
    int8u j;
    calendar->weeks[i].id = i;
    for (j = 0; j < EMBER_AF_DAYS_IN_THE_WEEK; j++) {
      calendar->weeks[i].daysArray[j] = (j < EMBER_AF_PLUGIN_TOU_CALENDAR_COMMON_DAY_PROFILE_MAX
                                         ? &(calendar->normalDays[j])
                                         : NULL);
    }
  }
}
...
static void setupTestSeasons(EmberAfTouCalendarStruct* calendar)
{
  ...
  for (i = 0; i < calendar->numberOfSeasons; i++) {
    if (calendar->weeks[i].id != EMBER_AF_PLUGIN_TOU_CALENDAR_COMMON_INVALID_ID) {
       calendar->seasons[i].week = &(calendar->weeks[i]);
    } else {
      calendar->seasons[i].week = NULL;
    }
  }
}
...
void emberAfPluginTouCalendarCommonPrintSeasonsCommand(void)
{
  ...
  for (i = 0; i < EMBER_AF_PLUGIN_TOU_CALENDAR_COMMON_SEASON_PROFILE_MAX; i++) {
    ...
    if (calendar->seasons[i].week == NULL) {
      emberAfTouCalendarClusterPrintln("  No data");
    } else {
      printTime("  Start Date:  ", calendar->seasons[i].startDate);
      emberAfTouCalendarClusterPrintln("  Week ID: %d", calendar->seasons[i].week->id);
    }
  }
}
