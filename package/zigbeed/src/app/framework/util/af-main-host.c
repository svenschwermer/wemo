...
void emAfResetAndInitNCP(void)
{
  ...
  // Seed the host side random number generator
  {
    ...
    if (((ezspGetRandomNumber(&seed_low)) == EMBER_SUCCESS) &&
        ((ezspGetRandomNumber(&seed_high)) == EMBER_SUCCESS)) {

      fprintf(stderr,
		      "ezspGetRandomNumber seed_high:0x%X\n", seed_high);
      fprintf(stderr,
		      "ezspGetRandomNumber seed_low:0x%X\n", seed_low);

      halStackSeedRandom(((int32u)seed_high << 16) | (int32u)seed_low);
    } else {
      // report status here
      fprintf(stderr,
                "ERROR: ezspGetRandomNumber\n");
    }
  }
...
void zbdWriteVersionToFile(EmberVersion versionStruct)
{
	const char * file_name = "/tmp/em357.ver";
	FILE *fp;

	fp = fopen(file_name, "w");
	if (fp)
	{
		fprintf(fp, "%d%d%d", versionStruct.major, versionStruct.minor, versionStruct.patch);
		fclose(fp);
	}
	return;
}
...
void emAfCliVersionCommand(void)
{
  ...
  if (EZSP_SUCCESS != ezspGetVersionStruct(&versionStruct)) {
    ...
    zbdWriteVersionToFile(versionStruct);
  }
  emberAfAppFlush();
}
