...
void zdoDeviceLeaveCommand(void);
void nwkDeviceLeaveCommand(void);
...
EmberCommandEntry zdoCommands[] = {
  ...
  emberCommandEntryAction("zdoleave",
                          zdoDeviceLeaveCommand,
                          "vu",
                          "Request zdo device leave"),
  emberCommandEntryAction("nwkleave",
                          nwkDeviceLeaveCommand,
                          "vb",
                          "Request nwk device leave"),
  emberCommandEntryTerminator(),
};
...
{
  EmberNodeId target = (EmberNodeId)emberUnsignedCommandArgument(0);
  int8u leaveRequestFlags = (int8u)emberUnsignedCommandArgument(1);
  ...
  emberAfAppPrintln("zdo Device Leave Command %x", status);
}

void nwkDeviceLeaveCommand(void)
{
  EmberNodeId target = (EmberNodeId)emberUnsignedCommandArgument(0);
  EmberEUI64 destinationEui64, tmp;

  emberCopyEui64Argument(1, tmp);
  emberReverseMemCopy(destinationEui64, tmp, EUI64_SIZE);

  EmberStatus status = emberSendRemoveDevice(target, destinationEui64, destinationEui64);

  emberAfAppPrintln("nwk Device Leave Command %x", status);
}
                                     ...
                                     EmberEUI64 returnEui64)
{
  ...
  if (0 == emberAfCopyBigEndianEui64Argument(4, returnEui64)) {
