...
void zclBufferSetup(int8u frameType, int16u clusterId, int8u commandId) 
{
  ...
  emberAfGuaranteedPrintln("zclBufferSetup appZclBufferLen %d",
			   appZclBufferLen);
}
                             ...
                             int8u commandId)
{
  ...
  for (i = 0; i < EMBER_MAX_COMMAND_ARGUMENTS; i++) {
    ...
    emberAfGuaranteedPrintln("zclSimpleCommand clus 0x%2x, cmd 0x%x, i %d, type: %c",
                           clusterId, 
                           commandId, 
                           i, 
			   type);
...
EmberStatus zbdZigBeeSendCommand(int16u destination, int8u srcEndpoint, int8u dstEndpoint, int16u flagMulticast)
{
	...
	if (flagMulticast)
	{
		status = emberAfSendMulticast(destination,
				...
				appZclBufferLen,
				appZclBuffer);
		emberAfGuaranteedPrintln("Multicast");
	}
	else
	{
