...
extern char g_routerMac[20];
extern char g_routerSsid[64];
extern char g_szRestoreState[2];
extern char g_szPluginPrivatekey[50];
...
{
	char cmd[100];
        char arg3[64],arg4[64],arg5[64],arg6[64],arg7[64];
	int cmdnum = -1;
...
{
    char* tempchar = GetBelkinParameter(DIMVALUE);
    if(0 == strlen(tempchar))
    {
	APP_LOG("APP", LOG_DEBUG, "-------->>>>> setting DIMVALUE"  );
	SetBelkinParameter (DIMVALUE,DEFAULT_DIMVALUE);
	AsyncSaveData();
	u32DimVal = atoi(DEFAULT_DIMVALUE);
    }
    else{
        u32DimVal = atoi(tempchar);
    }
    APP_LOG("APP", LOG_DEBUG, "-------->>>>> DIMVALUE : %d",u32DimVal );
    if((u32DimVal == 0) || (u32DimVal == 2))
	ChangeNightLight(u32DimVal);
}
