...
static int deinitialize_system(void)
{
  ...
  unlink(UDS_PATH);
   ...
   SE_INC_LOG(SEMAPHORE,
      ulog_safe(1, ULOG_SYSTEM, UL_SYSEVENT, "Destroying semaphore worker_sem\n");
   )
   ...
   ulog_safe(1, ULOG_SYSTEM, UL_SYSEVENT, "syseventd stopped\n");
   return(0);
}
...
static void terminate_signal_handler (int signum)
{
   ...
   ulog_safe(1, ULOG_SYSTEM, UL_SYSEVENT, "Received terminate signal\n");
   ...
   if (0 != main_communication_fd_writer_end) {
      ...
      if (NULL != body) {
         ...
         void *result;
         SE_INC_LOG(MUTEX,
            ulog_safe(1, ULOG_SYSTEM, UL_SYSEVENT, "Attempting to get mutex: main_communication\n");
         )
         pthread_mutex_lock(&main_communication_mutex);
         SE_INC_LOG(MUTEX,
            ulog_safe(1, ULOG_SYSTEM, UL_SYSEVENT, "Got mutex: main_communication\n");
         )
         for (i=0; i<numThreads; i++) {
         ...
         SE_INC_LOG(MUTEX,
            ulog_safe(1, ULOG_SYSTEM, UL_SYSEVENT, "Releasing mutex: main_communication\n");
         )
...
static void reinit_signal_handler (int signum)
{
   ...
   ulog_safe(1, ULOG_SYSTEM, UL_SYSEVENT, "Received reinit signal");
}
...
//  debugLevel = SHOW_ERROR|SHOW_SANITY;
   ...
   */
   ...
   if (0 == pid) {   /* child */
      ...
      execl("/sbin/"SYSEVENTD_FORK_HELPER_PROCESS, SYSEVENTD_FORK_HELPER_PROCESS, pipe_str, NULL);
      ulogf(ULOG_SYSTEM, UL_SYSEVENT, "Unable to exec fork helper process %s. (%d) %s",
            "/sbin/"SYSEVENTD_FORK_HELPER_PROCESS, errno, strerror(errno)) ;
   ...
   for (i=0; i<numThreads; i++) {
      ...
      if (0 != pthread_create(&(global_thread_id[i]), NULL,
   ...
   pthread_create(&sanity_thread_id, NULL, sanity_thread_main, (void *)NULL);
