...
all: 
	...
	$(BELKIN_TOOL)/makeImage $(BIN_DIR) $(KERNEL_FILE) $(RFS_FILE) $(BELKIN_BIN) $(BELKIN_FIRMWARE)-belkin.bin WeMo_$(SKU)
...
#	@echo "=== Making TRANSITION OpenWRT Firmware ==="
	$(BELKIN_TOOL)/fw_crc_hdr $(BELKIN_FIRMWARE)-belkin.bin $(BELKIN_FIRMWARE).bin
	gpg --homedir $(KEYS) -o $(BELKIN_FIRMWARE).bin.gpg.out -s $(BELKIN_FIRMWARE).bin 
	mv $(BELKIN_FIRMWARE).bin.gpg.out $(BELKIN_FIRMWARE).bin.dev.gpg
#endif
	@echo " "
	...
	@echo "----------------------------------------------------------------------------"
	@echo "1. With Belkin ImageInfo and Gemtek header:"
	@echo " $(BELKIN_FIRMWARE).bin" 
	@echo "2. With Belkin ImageInfo only: can be used for non-signed update" 
	@echo " $(BELKIN_FIRMWARE)-belkin.bin" 
	@echo "3. Signed by Belkin Development key: can be used for one shot firmware Update"
