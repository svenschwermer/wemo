	...
	IN OUT	ADD_HT_INFO_IE 		*pAddHtInfo)
{
	...
	//WNC0_Andrew fix the MCS 32 issue for WFA cert.
    ...
    {
        pAd->ApCliMlmeAux.HtCapability.MCSSet[4] = 0x0;
    }
	//WNC0
	...
	IN UCHAR ifIndex)
{
	...
	do
	{
		...
		if (pMacEntry)
		{
			...
			MaxSupportedRate = dot11_2_ra_rate(MaxSupportedRate);
				...
				)
			{
				...
				{
					AsicSetEdcaParm(pAd, &pAd->ApCliMlmeAux.APEdcaParm);
	...
	if (pAd->bApCliCertTest == TRUE)
	{
		if ((pAd->CommonCfg.bBssCoexEnable == TRUE)
		    ...
		    && (pAd->ApCliMlmeAux.ExtCapInfo.BssCoexistMgmtSupport == 1)) {
		...
		else
		{
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("pAd->ApCliMlmeAux.ExtCapInfo.BssCoexstSup %d !!! \n",
				  pAd->ApCliMlmeAux.ExtCapInfo.BssCoexistMgmtSupport));
			DBGPRINT(RT_DEBUG_TRACE,
	...
	IN	UCHAR			FromWhichBSSID)
{
	...
	*/
	...
	if ((pRxD->MyBss == 0) && (pRxD->Decrypted == 0) && (pHeader->FC.Wep == 1)
		&& pWpsCtrl->bWscTrigger == FALSE && pEntry->PortSecured==WPA_802_1X_PORT_SECURED) 
	{											
