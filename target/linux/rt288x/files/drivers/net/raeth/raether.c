...
// RT5350 need to reset ESW and FE at the same to avoid PDMA panic //	
   ...
   if(i < 0 || 
      memcmp(addr.sa_data, zero, 6) == 0 || 
      memcmp(addr.sa_data,mac_addr01234,3) == 0)
   {
   // If the MAC address OUI is Ralink's then the ODM didn't write a valid
   // MAC address into Flash.  Since the wired Ethernet port isn't normally
   // used on WeMo products this is understandable if undesireable.
   // In this case use RA0's MAC address and set the locally administered bit
#ifdef RA_MTD_RW_BY_NUM
      i = ra_mtd_read(2,RA0_MAC_OFFSET,6,addr.sa_data);
#else
      i = ra_mtd_read_nm("Factory",RA0_MAC_OFFSET,6,addr.sa_data);
#endif
      addr.sa_data[0] |= 0x2; // set the locally administered bit
   }
...
// Belkin patch: Disable VLAN on *ALL* ports, including CPU
        *(unsigned long *)(0xb0110098) = 0x00007f7f; //disable VLAN
...
//+++Gemtek add
	mii_mgr_write( 0, 0, 0x3100);
        mii_mgr_write( 1, 0, 0x3100);
        mii_mgr_write( 2, 0, 0x3100);
        mii_mgr_write( 3, 0, 0x3100);
        mii_mgr_write( 4, 0, 0x3100);
//---Gemtek add
	...
	*(unsigned long *)(0xb0000060) &= ~(1 << 9); //set RGMII to Normal mode
    ...
    /*   GTK mark GPIO22~GPIO27 is Big LED and Small LED (Initial on ralink_gpio.c)        
        *(unsigned long *)(0xb0000674) = 0xFFF; //GPIO22-GPIO27 output mode
        *(unsigned long *)(0xb0000670) = 0x0; //GPIO22-GPIO27 output low
    */        
...
UINT32          Chk_IfTheSpecificBitUp32Bits ( UINT32 orig , int bitnum )
{
        UINT32          i = 0x0000;
...
/* 0 : no linked, 1 : linked */
 unsigned char           GetPHYLinkStatus ( int phyUnit )
{
        int                             PortNum = 0;
        esw_reg         reg;

        if ( ( phyUnit < 0 ) || ( phyUnit > 4 ) ) {
                printk ( "GetPortLinkStatus : phyUnit ( %d ) is invalid\n" , phyUnit );
                return  0;
        }

        /* RaLink_RT_3052_DataSheet_DSRT3052_V1.0_050508.pdf, Page # 121 */
        ...
        if ( 0x00000000 < Chk_IfTheSpecificBitUp32Bits ( reg.val , PortNum ) ) { /* 0 : Link fail, 1 : Link Pass */
                return  1;
        } else {
                return  0;
        }
...
static int lanLink_read_proc(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
	int len = 0;
	//uint32 i, status_lan, status_all = 0;
	u32 i, status_lan, status_all = 0;

	for(i = 0;i <= 3;i++)
	{
		status_lan = GetPHYLinkStatus(i);
		//status_lan = 1;
	  if(status_lan == 1)
	  {
//	  	printk(KERN_EMERG "LAN%d : Connect!\n", i);
	  	status_all |= 1;
	  }
	  else
	  {
//	  	printk(KERN_EMERG "LAN%d : Disconnect!\n", i);
	  	status_all |= 0;
	  }
	}
  ...
  if(status_all == 1)
  {
//  	printk(KERN_EMERG "All LAN : Connect!\n");
  	len = sprintf(buf,"1");
  	//************Eric add note:
  	//************It may should be added GPIO LCD on/off .
  }
  else
  {
//  	printk(KERN_EMERG "All LAN : Disconnect!\n");
  	len = sprintf(buf,"0");
  	//************Eric add note:
  	//************It may should be added GPIO LCD on/off .
  }		
...
static int wanLink_read_proc(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
	...
	//status_wan = GetPHYLinkStatus(0);
	// For RT3050, WAN = P4
	status_wan = GetPHYLinkStatus(4);
	//status_wan = 1 ;
//	printk(KERN_EMERG "Wan link status is : %d\n", status_wan);

	if(status_wan  == 1)	// wan port link state, phy 4, reg 1, bit 2
	{
		len = sprintf(buf,"1");
	}else{
		len = sprintf(buf,"0");
...
//+++Gmtek Add
	...
	create_proc_read_entry("lanLink",0,NULL,lanLink_read_proc,NULL);
//---Gemtek Add
	return ret;
}
